...
def _mktdata(values: list[float]) -> dict[str, pd.DataFrame]:
    ...
    return {"AAPL": pd.DataFrame({"close": values}, index=idx)}


def test_peak_detects_local_max():
    # order=1: a peak is higher than its immediate neighbors
    result = Peak().compute(_mktdata([10, 20, 15, 25, 10]), column="close", kind="peak", order=1)
    assert list(result["AAPL"]) == [False, True, False, True, False]


def test_trough_detects_local_min():
    result = Peak().compute(_mktdata([20, 10, 25, 5, 15]), column="close", kind="trough", order=1)
    assert list(result["AAPL"]) == [False, True, False, True, False]
...
def test_peak_order_2():
    ...
    result = Peak().compute(
        _mktdata([10, 15, 30, 20, 10, 5, 25]),
        column="close", kind="peak", order=2,
    )
    vals = list(result["AAPL"])
...
def test_peak_edges_are_false():
    ...
    result = Peak().compute(_mktdata([100, 50, 100]), column="close", kind="peak", order=1)
    assert list(result["AAPL"]) == [False, False, False]


def test_peak_multi_symbol():
    idx = pd.bdate_range("2024-01-01", periods=3)
    mktdata = {
        "A": pd.DataFrame({"close": [10, 20, 10]}, index=idx),
        "B": pd.DataFrame({"close": [20, 10, 20]}, index=idx),
    }
    result = Peak().compute(mktdata, column="close", kind="peak", order=1)
    assert list(result["A"]) == [False, True, False]
    assert list(result["B"]) == [False, False, False]
