...
Agent First quantitative trading framework built by your operator. It is in early
Pre-Alpha stage. The core pipeline:
```
Data → Factor → Signal → Backtest → Paper Trading → Live Trading
```

Execution model: `Indicator → Signal → Rule` three-phase architecture.
The MCP server exposes the full `oxq` SDK as callable tools.
...
- Tools or MCP calls that feel awkward or incomplete
