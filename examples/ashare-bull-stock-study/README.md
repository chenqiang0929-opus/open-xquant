...
上面那条纪律(判据事前写死、事后不放宽)已经执行 **48 次,一次没放宽过**。
§113 是最能说明这条纪律有用的一次:判据 C1 不通过,而**不通过的原因在被复现对象一侧**
(Codex 把前复权价乘股本当市值),我照样按判据把本节判为作废、另开 §114 重新事前登记,
没有把「其实我是对的」写成「所以判据算过」。
**但它防的是「事后改标准」,防不了「事前就把标准写错」—— 后者已经发生八次。**

**它也防不了「事前预测错」。事前预测被证伪累计 14 次**,其中 §113–§118 一轮就占 8 次;
最常见的一种错法出现了 **8 次**:**把「样本内信号强」外推成「样本外会过」**
(§98/§103/§104/§105/§112/§115-B P3/§116-A Q1/§118 T4)。
最刺眼的证据是 §116-A:`small_cap` 在 full 窗口 p=0.0020(500 组同市值随机对照
**没有一组**跑赢),在唯一没被看过的 2026 留出期却是 **−18.16%**。
