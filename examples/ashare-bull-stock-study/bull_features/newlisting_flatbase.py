...
⚠️ **本脚本从未运行过。** 写它的那一轮容器里面板数据已丢失
   (`oxq_stock_market_fixed/` 不存在)。所以它没有任何实测数字,
   也**不构成任何收益声明**。第一次运行时若锚点对不上,请先修锚点再看结论。
...
═══ 三个必须堵住的陷阱(每个都会单独制造假阳性) ═══
① **面板起点会伪造出一批假次新股。**
   用「首个有效交易日」推上市日,必然把面板起点之前上市的老股票
   全部误判成起点当天上市的次新股。而第五十四节已证:整套突破系统的收益
   几乎全部来自 2013-04~2015-05(那段 fwd_gain 均值 +117.09%,之后只有 +48.22%)。
   **这两个错误会叠加成一个极其漂亮的假阳性。**
   → 硬性排除 first_valid ≤ 面板起点 + GUARD_DAYS 的标的,并报出排除了多少只。
...
# ── 事前锁定的两个新参数,不搜索 ──
AGE_LO, AGE_HI = 250, 750      # 上市年龄窗口(交易日)≈ 1~3 年
GUARD_DAYS = 60                # 陷阱①:面板起点后 60 个交易日内「首现」的一律剔除
...
first_valid = np.full(CLa.shape[1], -1, dtype=np.int64)
for j in range(CLa.shape[1]):
    w = np.flatnonzero(ALIVE[:, j])
    if w.size:
        first_valid[j] = int(w[0])
has_data = first_valid >= 0
# 「首现」落在面板最开头的,几乎肯定是面板起点前就已上市的老股票 —— 一律剔除。
# 不剔除的话,它们会被当成 2013 年初的次新股,而突破系统的收益恰好几乎全部
# 来自 2013-04~2015-05,两个错误叠加会产出一个极漂亮的假阳性。
truncated = has_data & (first_valid <= GUARD_DAYS)
usable_stock = has_data & ~truncated
print(f"\n上市日期代理:有数据 {has_data.sum():,} 只;"
      f"其中「首现 ≤ 第 {GUARD_DAYS} 个交易日」判定为面板截断、剔除 **{truncated.sum():,}** 只;"
      f"可用 **{usable_stock.sum():,}** 只")
print("  ⚠️ 这是代理,不是真实上市日。若有外部上市日期表,应替换本段。")

_t = np.arange(NT, dtype=np.int64)[:, None]           # AGE[t, j] = t - 首个有效日
AGE = np.where(usable_stock[None, :], _t - first_valid[None, :], -1)
IS_NEW = (AGE >= AGE_LO) & (AGE <= AGE_HI) & ALIVE    # 次新 = 上市 1~3 年且在市
...
print(f"  次新(上市 {AGE_LO}~{AGE_HI} 日)      {new_.sum():>7,} ({new_.mean():>6.2%})")
