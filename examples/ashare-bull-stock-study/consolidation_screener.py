...
改造修好了旧口径的两个毛病:
  ...
  - 用户 19 只案例在**事件管线**上回归全过,并新捞到 6 只

**但在本筛选器的「逐日状态」口径下,分位阈值反而更严、案例覆盖更差**
(`screener_case_coverage.py`,19 只案例全历史):

    档                                     亮过灯   总亮灯天数
    A 旧版(绝对阈值 + MA100向上 + 写死15日)   18/19    4,317
    B 去掉MA100向上 + 自适应下限,仍用绝对阈值   19/19    4,573
    C 新版默认(再换成当期40%分位)             17/19    **1,130**
