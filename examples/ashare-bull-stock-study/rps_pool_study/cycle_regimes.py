...
# =============================================================================
# §126 结果  锚点 X1 ✓  X2 曲线一致性超差 0 条 ✓(与 §122 逐条相符)
#
# 主口径 ZigZag 20%,沪深300 月末收盘(含分红),切出 8 段:
#   2014-03-31 → 2015-05-31 牛  14月 300:+127.32% | 股票最优 R10 +221.13%
#   2015-05-31 → 2015-09-30 熊   4月 300: -35.61% | 股票最优 R04  -1.09%
#   2015-09-30 → 2015-12-31 牛   3月 300: +16.41% | 股票最优 R09 +41.88%
#   2015-12-31 → 2016-02-29 熊   2月 300: -22.86% | 股票最优 R04  +0.14%
#   2016-02-29 → 2018-01-31 牛  23月 300: +53.02% | 股票最优 R08 +59.37%
#   2018-01-31 → 2018-12-31 熊  11月 300: -28.26% | 股票最优 R12 -17.35%
#   2018-12-31 → 2021-01-31 牛  25月 300: +85.43% | 股票最优 R13 +67.57%
#   2021-01-31 → 2022-10-31 熊  21月 300: -33.08% | 股票最优 R10 +77.84%
#   2022-10-31 → 2023-01-31 牛   3月 300: +18.19% | 股票最优 R03 +31.34%
#   2023-01-31 → 2024-01-31 熊  12月 300: -21.04% | 股票最优 R08 +10.74%
#   2024-01-31 → 2026-07-31 牛(进行中) 30月 300: +53.54% | 股票最优 R10 +169.04%
# (25% 阈值切出 8 段、把 2021-2024 合成一段 36 个月的熊市,方向与 20% 一致)
#
# ── 一致性:冠军几乎不重复,不可外推 ──
# 牛市 4 段 → R10×2, R08×1, R13×1
# 熊市 4 段 → R05×1, R04×1, R12×1, R10×1
# **熊市四段四个不同的冠军。** 这正是事前登记里警告的形态:
# 13 条路线 × 4 段,总有人排第一,那是噪声不是信号。
# **「上一轮熊市 R04 最好」对下一轮没有任何预测力。**
#
# ── 跑赢沪深300 的次数才是稳的统计(不受其他 12 条影响)──
# 路线  牛 x/4  熊 y/4        路线  牛 x/4  熊 y/4
# R03    2/4     4/4          R09    1/4     4/4
# R04    0/4     4/4          R10    2/4     4/4
# R05    1/4     3/4          R11    2/4     4/4
# R06    1/4     3/4          R12    2/4     4/4
# R07    0/4     2/4          R13    0/4     3/4
# R08    2/4     4/4          R01    0/4     4/4(择时,熊市可空仓)
#                             R02    0/4     4/4(择时,熊市可空仓)
#
# **最刺眼的规律:熊市跑赢 4/4 的有七条,牛市跑赢 4/4 的一条都没有,最高只有 2/4。**
# 而且 R04(超卖反转,全期年化仅 +2.04%)熊市 4/4、牛市 0/4 ——
# 它不是好策略,它只是**跌得比指数少**。
# 这说明这些路线的共同形态是**防守型**:熊市少亏、牛市跟不上。
# 对「牛市该用谁」这个问题,**本节给不出答案** —— 没有一条在牛市里稳定占优。
#
# ── 必须一起读的三条(事前登记里已写死)──
# ① 每轮冠军不可外推。熊市四段四个冠军,已经自证。
# ② R01/R02 是 510300 择时,**熊市可空仓** → 熊市 4/4 是结构性的,
#    与 R03–R13 的满仓股票组合**不可直接比**。
# ③ R08 在 2023-01→2024-01 那段熊市里 +10.74% 排第一,
#    但 §125 已测出它 56.5%(2021–2026 那段 75.8%)是银行 ——
#    **那一段正是银行/高股息走强的时候**。它在另外三段熊市里没有夺冠,
#    符合「不可外推」的判断。
#
# ── 第一版的 bug(如实记录)──
# 第一版 ZigZag 的 `direction=0` 分支写错,max/min 两套逻辑同时生效把 `ext`
# 覆盖成当前价,阈值条件永远不成立,13 年只切出 1 段,且三个阈值结果完全相同。
# 是「1 段」这个明显不合理的输出暴露了它,不是锚点抓到的 ——
# **X2 只验了净值曲线,没验切分本身,这是本节锚点设计的一个漏洞。**
# =============================================================================
