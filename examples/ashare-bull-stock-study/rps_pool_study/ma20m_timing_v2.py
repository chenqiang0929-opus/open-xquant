"""第九十六节:20 月线择时全样本 **重跑** —— §95 因锚点作废,锚点值本身是错的(事前登记)

═══ 为什么重跑:§95 的锚点期望值是错的,不是脚本错 ═══
§95 锚点② 要求宇通 MA20 复现「总收益 +528.7% / 回撤 **20.7%** / 交易 11 次」。
实测 **+528.7% / 35.8% / 11 次** —— 收益与交易次数一字不差,**只有回撤对不上,
因为那个 20.7% 本身是错的**。`case_yutong_ma20m.py` 把最大回撤写成了

    (np.maximum.accumulate(eq) - eq).max() / np.maximum.accumulate(eq).max()

这是「最大绝对跌幅 ÷ 全局峰值」,早期回撤被后期高净值稀释,**系统性低估**。
合成校验 `eq=[1,2,1.2,5,4]`:旧 **20.0%**,正确 **40.0%**。
**该 bug 由 §95 的全样本锚点抓出 —— 本项目第一次由全样本锚点抓出单只脚本的 bug。**

公式已修正并重算,宇通正确数字:

    买入持有   +541.9%  年化 +14.6%  最大回撤 **65.0%**(原报 29.5%,错)
    MA20 月线  +528.7%  年化 +14.4%  最大回撤 **35.8%**(原报 20.7%,错)
    MA12 月线  +466.1%  年化 +13.5%  最大回撤 **25.5%**

**修正后 20 月线的降回撤效果比原报的更强(65.0%→35.8%),不是更弱。**

**§95 的三条判据结果一律不复用**(那一节已作废)。本节判据与 §95 逐字相同,
**只改锚点② 的期望值**,以便与 §95 的作废记录直接对照。
...
═══ 锚点(不过则全节作废;三个都是恒等式)═══
  ① 面板 (3297, 5232)
  ② **宇通恒等复现单只跑(锚点值已修正)**:MA20 总收益 **+528.7%**、
     最大回撤 **35.8%**、**11** 次交易(容差:收益 ±0.5pp、回撤 ±0.5pp、次数必须一致)
...
a2 = abs(jr - 5.287) <= 0.005 and abs(jd - 0.358) <= 0.005 and jt == 11
print(f"  {'✓' if a2 else '✗'} 锚点② 宇通 MA20 恒等复现:总收益 {jr:+.1%}(期望 +528.7%)、"
      f"回撤 {jd:.1%}(期望 35.8%)、交易 {jt} 次(期望 11)")
if not a2:
    bad.append("锚点②")
...
pd.DataFrame(rows).to_csv(f"{OUT}/ma20m_timing_v2.csv", index=False)
Y.to_csv(f"{OUT}/ma20m_timing_v2_yearly.csv", index=False)
print(f"\n→ {OUT}/ma20m_timing_v2.csv + _yearly.csv   ({time.time()-t0:.0f}s)")
