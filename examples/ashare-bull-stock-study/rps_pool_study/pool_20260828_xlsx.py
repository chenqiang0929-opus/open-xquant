"""§165 交付(次新股池版):2026-08-28 当日清单 + 锚点,整合成带分组表头与配色的 Excel。
...
COLS = [c for _, cs in GRP for c in cs] + ["数据状态"]
FILL = {
...
def main():
    a = pd.read_csv(f"{OUT}/pool_20260828.csv", dtype={"股票代码": str})
    ...
    lines = [
        ("2026-08-28 次新股池清单(662 只全量)—— RPS50 分档,本地全市场面板", 0),
        (f"共 {len(a):,} 行、{a['股票代码'].nunique():,} 只｜"
         f"观察日 2026-08-28｜次新股池 662 只全量(含无信号与数据不足)｜面板 3,316 交易日 × 5,232 只｜行情与财务已更新至 2026-08-28", 1),
        ("", 0),
