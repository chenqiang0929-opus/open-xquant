...
# =============================================================================
# §125 结果:**W2 通过 3/3。我提出的怀疑没有成立 —— 超额扛住了行业中性。**
#
# 锚点 W1a ✓  W1b ✓ 行业恒等式违例 0 次  W1c ✓ 行业覆盖率 100.0%  W1d ✓ TTM
# 行业矩阵:31 个申万一级行业,PIT 生效区间。
#
# 策略        银行占比 | 训练年化  行业中性对照  p     | 留出年化  回撤    夏普  对照   p      W2
# R08          56.5%  | +15.12%    +7.15%   .0020 |  +9.98% -22.67% 0.67 +3.57% .0020  ✓
# R09           7.7%  |  +8.73%    +0.23%   .0040 |  +6.63% -22.22% 0.49 -3.10% .0020  ✓
# CFP_single   23.8%  | +16.63%    +6.02%   .0020 | +11.57% -23.07% 0.64 +2.36% .0020  ✓
#
# ── 最有价值的一点:超额被干净地拆成了两段 ──
# 以 R08 的留出期为例,三个对照并排:
#   全市场同市值随机对照      **-1.57%**   (§119)
...
#   R08 本身                  **+9.98%**
# 差值给出分解:
#   **行业暴露(主要是银行)贡献 ≈ 5.14pp**
#   **行业内的选股贡献       ≈ 6.41pp**
# 两段都是实的。我原先担心「超额全是银行 beta」—— 数据说不是,
# **行业 beta 只占一半不到,另一半是行业内选股。**
#
# ── R09 是更强的反证 ──
# R09 的银行占比只有 **7.7%**,根本不是银行押注,却同样通过
# (留出 +6.63% vs 行业中性对照 **-3.10%**,p=.0020)。
# 若「价值/质量的超额=银行 beta」成立,R09 不该通过。
#
# ── W3 对「闭眼买银行」的比较 ──
#   等权全部合格银行股(不含成本、不含整手):训练 +9.94% 回撤 -37.57% 夏普 0.53
#                                            留出 +7.76% 回撤 -17.31% 夏普 0.55
#   R08(含全部成本与整手):                 留出 +9.98% 回撤 -22.67% 夏普 0.67
#   **R08 比闭眼买银行高 2.22pp,而且是在自己付成本、对方不付成本的情况下。**
#   但银行等权的**留出期回撤只有 -17.31%**,优于 R08 的 -22.67% —— 这一格是银行赢。
#
# ── 我提出怀疑、检验、然后被数据否定,过程如实记录 ──
# 我在看到持仓名单时判断「R08 不是价值策略,是银行板块押注」,
# 并说「§119/§123/§124 对 R08 的结论需要打问号」。
# **本节直接检验了这个怀疑,结论是:怀疑不成立,那三节的判定继续有效。**
# 提出怀疑是对的(八轮没做行业中性确实是疏漏),但**结论下早了** ——
# 我在没有行业数据、只凭代码识别时就说「严重削弱推荐」,应该等测完再说。
#
# ── 但有一条仍然成立,不要混淆 ──
# **「超额是真的」和「集中度是风险」是两件事。**
# R08 全期银行占比 56.5%、2021–2026 那一段 75.8%、最新一期 20/20 全是银行,
# 67 次调仓只出现过 100 只不同股票。**本节证明的是超额不虚,
# 没有证明这个集中度可以承受。** 单一板块的政策/监管/利率风险是本节测不到的。
# =============================================================================
