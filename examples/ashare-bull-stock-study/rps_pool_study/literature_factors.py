...
# =============================================================================
# §123 结果:文献因子 2/6 通过;通过的两条 + R08/R09 全部扛住退市清算与双倍成本。
#
# 锚点 N1a ✓ 面板  N1b ✓ 抽样越界 0  N1c ✓ TTM 恒等式
#      N1d ✓ 泰格同比  N1e ✓ 成本引擎恒等式(train/holdout 最大相对误差 0.000e+00)
# 退市清算口径:判定已退市 238 只,其后价格 ×0.10。
#
# 因子(出处)              训练年化  训练p   留出年化  留出回撤 夏普  对照中位  留出p    N2
# BM_FF1993   (FF 1993)   +17.18%  .0080    -1.16%  -31.07% 0.06   -2.09%  .3792   ✗
# EP_Basu1977 (1977)      +14.09%  .0220    +4.19%  -25.83% 0.33   -2.57%  .0220   ✗
# CFP_LSV1994 (1994)      +16.83%  .0020   +11.57%  -23.07% 0.64   -0.81%  .0020   ✓
# SP_BMR1996  (1996)      +22.56%  .0020    +8.67%  -29.64% 0.50   -0.95%  .0060   ✓
# ROE_HB1996  (1996)      +12.95%  .0180    +1.28%  -30.66% 0.16   -2.98%  .0978   ✗
# ACCRUAL_Sloan1996v(变体)+13.12%  .0319    +6.77%  -35.08% 0.40   -0.62%  .0339   ✗
# R08(参照)               +15.32%  .0040    +9.98%  -22.67% 0.67   -1.57%  .0020   —
# R09(参照)                +8.75%  .0180    +6.63%  -22.22% 0.49   -2.51%  .0060   —
#
# N2 通过 2/6(α=0.008333):**CFP_LSV1994、SP_BMR1996**
#
# N3 退市按 -90% 清算(留出期,200 组对照)
#   CFP +11.57% 对照 -1.31% p=.0050 ✓ | SP +8.67% 对照 -0.83% p=.0050 ✓
#   R08  +9.98% 对照 -1.34% p=.0050 ✓ | R09 +6.63% 对照 -2.83% p=.0050 ✓
#   **四条全过,而且年化几乎不变** —— 这四条选的是中大盘,留出期几乎不碰退市股,
#   所以「冻结在最后有效价」这个已知偏差对它们**不构成实质影响**。
#   (对照 CFP 的对照中位从 -0.81% 降到 -1.31%,说明退市清算主要打在随机对照上。)
#
# N4 双倍成本(佣金/印花/滑点全部 ×2,留出期,200 组对照)
#   CFP +9.72%(-1.85pp)| SP +7.89%(-0.78pp)| R08 +8.75%(-1.23pp)| R09 +5.10%(-1.53pp)
#   **四条全过 p=.0050。** 成本翻倍吃掉 0.8~1.9pp 年化,超额仍在。
#   注意对照中位从 -1% 量级掉到 -6% 量级 —— **成本对随机对照的打击远大于对策略**,
#   因为策略的组合换手率更低。
#
# ── 这一节到底证明了什么 ──
# **挑选偏差这个洞,堵上了一半。**
# CFP(Lakonishok-Shleifer-Vishny 1994)与 SP(Barbee-Mukherji-Raines 1996)
# 是三十年前就写死的定义,**谁都不可能是看着这份 A 股数据挑出来的**。
# 它们在 2022–2026 留出期上打得过同市值随机对照(p=.0020 / .0060),
# 且扛住退市清算与双倍成本。**所以「A 股价值因子有超额」这件事,
# 不是 Codex 那 130 次搜索搜出来的,它独立于他的搜索存在。**
#
# 但只堵上一半,另一半没堵:
# **B/M 没过(留出期 p=0.3792,年化 -1.16%)** —— 而 B/M 恰恰是 Fama-French
# 的正宗价值因子。这说明 A 股这一段里,**「便宜」要用现金流和营收来衡量,
# 用账面价值衡量是失效的**。R08 的三个分量里 bp 是其中之一,
# 它对 R08 的贡献大概率是负的 —— 这一条本节没拆,是下一步该做的。
# **ROE 也没过(p=0.0978)**,而 ROE 是 R09 四项里的一项。
# 所以 R09 的成绩来自哪一项,同样没拆开。
#
# ── 边界 ──
# ① 本节只测单因子,**没有做任何复合**(§117 已测出 R11 朴素合成后 oos p=0.3333)。
#    CFP/SP 与 R08/R09 高度相关(CFP 就是 R08 的分量之一),**不是独立发现**。
# ② Novy-Marx (2013) 毛利率 GP/A 跑不了(面板无 COGS 与总资产),
#    质量族缺了最主流的那一个,**质量这边的证据比价值这边弱**。
# ③ 应计只能用每股口径变体,不是 Sloan 原定义。
# ④ 夏普 0.50~0.67、回撤 -23%~-30%,仍然不是「稳定盈利」。
# =============================================================================
