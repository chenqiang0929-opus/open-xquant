"""§163 交付:把 2022 清单与 272 行锚点整合成一个带分组表头与配色的 Excel。

**本脚本不做任何计算,只排版** —— 读 `codex_template_2022_v2.csv` 与
`codex_template_v2_anchor.csv`,输出 `codex_template_2022.xlsx`。
...
def sheet(wb, name, df, note=None):
    ...
    for k, col in enumerate(df.columns, 1):
        ...
        if col in ("RPS60", "RPS250", "R09核心质量分", "平台缩量比", "平台收敛比"):
            for rr in range(r0 + 2, r0 + 2 + n):
...
def main():
    a = pd.read_csv(f"{OUT}/codex_template_2022_v2.csv", dtype={"股票代码": str})
    a["股票代码"] = a["股票代码"].str.zfill(6)
    a = a[COLS]
    k = pd.read_csv(f"{OUT}/codex_template_v2_anchor.csv", dtype={"代码": str})
    ...
    lines = [
        ("2022 年带信号清单 —— 按 Codex 模板口径,本地 13 年全市场面板复现", 0),
        (f"共 {len(a):,} 行、{a['股票代码'].nunique():,} 只｜"
         f"样本 = 每月最后交易日 + 平台突破日｜面板 3,297 交易日 × 5,232 只", 1),
        ("", 0),
        ("■ 口径已与 Codex 对齐(2026-08-28 回函)", 0),
        ("信号类型  272/272 = 100.0%  ✓", 1),
        ("平台信号  271/272 =  99.6%  ✓", 1),
        ("周线五态  272/272 = 100.0%  ✓(修正前 82.4%,按回函改成有序判别后全中)", 1),
        ...
        (f"平台突破日      {int((a['样本类型']=='平台突破日').sum()):,} 行", 1),
    ...
    sheet(wb, "平台突破日", a[a["样本类型"] == "平台突破日"])
    sheet(wb, "全部清单", a)
    m = a[a["样本类型"] == "月末观察"].groupby(
        ["观察日期", "信号类型"]).size().unstack(fill_value=0)
    ...
    chk = pd.DataFrame([
        ...
        {"检查项": "月末观察行", "值": int((a["样本类型"] == "月末观察").sum())},
        {"检查项": "平台突破日行", "值": int((a["样本类型"] == "平台突破日").sum())},
    ...
    f = f"{OUT}/codex_template_2022.xlsx"
    wb.save(f)
    print(f"已生成 {f}")
