"""第九十八节:业绩反转 **重跑** —— §97 的锚点③ 自己算错了(事前登记)

═══ 为什么重跑:锚点③ 不过,是锚点代码错,不是数据错 ═══
§97 锚点③ 实测「roe 变更 6,078,740 次,落在 6/12 月 15.91%」,远超 2% 门槛,本节作废。
**但那个 15.91% 是我的锚点代码算出来的垃圾。**

    chg = np.diff(ROE, axis=0) != 0        # <- 错

**`NaN != 0` 在 numpy 里是 `True`。** 未上市期的 NaN 段被**逐日**计成「变更」——
5232 只 × 13 年季报应约 27 万次,实测 608 万次 = 每股 1162 次 ≈ 1/3 的交易日。
NaN 段在月份上近似均匀 → 6+12 月占 2/12 = **16.7%**,实测 **15.91%**,吻合。

**修正:两侧都有效才算变更**

    prev, cur = ROE[:-1], ROE[1:]
    chg = np.isfinite(prev) & np.isfinite(cur) & (cur != prev)

**修正后全市场预验证(锁定前先跑,§89 立的规矩):**

    roe 变更 **178,452** 次(平均每只 34 次,13 年季报应约 53 次,量级合理)
    落在 6/12 月 **477** 次 = **0.27%**  ✓ 远低于 2% 门槛
    正好落在 6-30 / 12-31 仅 **30** 次 = **0.017%**

**逐只抽样 400 只另行核对:92.5% 的股票 6/12 月变更为 0,中位 0.0%、均值 0.2%,
且那些变更散落在 6-14 / 12-8 / 6-10 等各种日子上,不集中在报告期末。**
**数据确实按公告日打戳,无前视。§97 的作废理由不成立,但 §97 不追认、结果不复用。**

**这是我第 10 次自查出的错误**(锚点自身实现错),也是本 session 第三次锚点抓到东西 ——
前两次抓的是真问题(§95 的 mdd 公式、§96 的期望值),**这次抓的是它自己。**

**本节判据与 §97 逐字相同,只改锚点③ 的计算方式。**
...
═══ 锚点(不过则全节作废;三个都是恒等式)═══
  ...
  ③ **公告日校验(计算方式已修正)**:全市场 roe 变更日落在 **6 月或 12 月**的
     比例 **< 2%**。变更判定必须**两侧都有效**(`isfinite(prev) & isfinite(cur) &
     (cur != prev)`)—— §97 用 `np.diff(...) != 0`,把 NaN 段逐日计成变更,
     算出 15.91% 的垃圾。**修正后预验证实测 0.27%,已知可过。**
...
# 锚点③ 公告日校验
ROE = MAT["roe"]
_prev, _cur = ROE[:-1], ROE[1:]
chg = np.isfinite(_prev) & np.isfinite(_cur) & (_cur != _prev)   # 两侧都有效才算变更
...
R.to_csv(f"{OUT}/earnings_turn_v2.csv", index=False)
Y.to_csv(f"{OUT}/earnings_turn_v2_yearly.csv", index=False)
print(f"\n→ {OUT}/earnings_turn_v2.csv + _yearly.csv   ({time.time()-t0:.0f}s)")
